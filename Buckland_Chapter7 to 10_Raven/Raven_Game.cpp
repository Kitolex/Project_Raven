...
{ 
  ...
  for (curBot; curBot != m_Bots.end(); ++curBot)
  {
    ...
    if ((*curBot)->isSpawning() && bSpawnPossible)
    {
	  ...
	  if((*curBot) == m_player){
		  m_player->TakePossession();
		  m_pSelectedBot = m_player;
	  }
    }
...
void Raven_Game::Render()
{
 ...
 // gdi->TextAtPos(300, WindowHeight - 70, "Num Current Searches: " + ttos(m_pPathManager->GetNumActiveSearches()));
  ...
  if (m_pSelectedBot)
  {
   ...
   if (IS_KEY_PRESSED(VK_SHIFT) && m_pSelectedBot->isPossessed())
    {
...
void Raven_Game::MoveUp(bool active){
	up = active;
	//m_player->MoveUp();
}

void Raven_Game::MoveDown(bool active){
	down = active;
	//m_player->MoveDown();
}

void Raven_Game::MoveLeft(bool active){
	left = active;
	//m_player->MoveLeft();
}

void Raven_Game::MoveRight(bool active){
	right = active;
	//m_player->MoveRight();
}